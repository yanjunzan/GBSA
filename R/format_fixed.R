format_fixed <- function(pedigreeTable, vcf.file, pathout,Core=5,pattern=paste("(^F2_",of_id,".*)|(^",of_id,"_F2.*)",sep="")) { #generate.input = TRUE
  if(!require(dplyr))
    require(dplyr)
  ...
  py <- paste("python3 ",system.file(package="GBSA"),"/Read.vcf.py",sep="")
  ...
  callScript <- function(family){
    cmd <- paste(py, "--ID_of", paste(family$of_id, collapse = " "), "--", family$F_id_h, family$M_id_l, vcf.file, pathout,sep = " ")
    print(cmd)
    system(cmd)
  }
